...
namespace Market.ViewModel.DataVM
{
    ...
    public class CategoryVM : BaseVM
    {
        ...
        /// Gets all categories.
        /// </summary>
        public static List<CategoryVM> GetAll
        {
            get
            {
                var allCategories = CategoryMapper.Mapper.UnPack(MarketProxy.Proxy.GetData("Category", Middleware.MethodType.GetAllItems, string.Empty, Guid.Empty));
                return (from c in allCategories select new CategoryVM(c)).ToList();
            }
        }
