...
namespace Market.Entities
{
    ...
    public class Salesperson
    {
        ...
        /// </summary>
        public User User
        { get { return UserReader.Reader.GetById(_userId)[0]; } }
        ...
        /// </summary>
        public List<Merchandise> SellingMerchandise
        {
            get
            {
                var res = from m in SDMs.AsEnumerable() select m.Merchendise;
                return res.ToList();
            }
        }
        ...
        public List<Dealer> Dealers
        {
            get
            {
                var res = from d in SDMs.AsEnumerable() select d.Dealer;
                return res.GroupBy(d => d.ID).Select(g => g.First()).ToList();
            }
        }
        ...
        public List<Bank> Banks
        {
            get
            {
                var res = (from bs in BankSalespersonReader.Reader.GetByQuery("SalespersonID='" + ID + "'").AsEnumerable()
                           select bs.Bank).ToList();
                return res;
            }
        }
        ...
        { get { return SpotReader.Reader.GetByQuery("SalespersonID='" + ID + "'"); } }

        /// <summary>
        /// Gets a list of clients and order details.
        /// </summary>
        public List<CustomerSelectedItems> Clients
        {
            get
            {
        ...
        public void Save()
        {
            SalespersonReader.Reader.Save(this, _isNew);
            if (this.SDMs.Count > 0)
                foreach (SDM sdm in this.SDMs)
                {
                    SDMReader.Reader.Save(sdm, sdm.IsNew);
                    sdm.IsNew = false;
                }
        }
        ...
        private List<CustomerSelectedItems> UpdateClients()
        {
            ...
            var all = CustomerSelectedItemsReader.Reader.GetByQuery(q);
